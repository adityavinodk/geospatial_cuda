...
std::vector<int> search_quadrant(const std::vector<Query> &queries, const std::vector<QuadrantBoundary> &boundaries)
{
    ...
    cudaMemcpy(d_queries, queries.data(), queries.size() * sizeof(Query), cudaMemcpyHostToDevice);
    cudaMemcpy(d_boundaries, boundaries.data(), boundaries.size() * sizeof(QuadrantBoundary), cudaMemcpyHostToDevice);

    int block_size = 256;
    int num_blocks = 16;
    quadrant_search<<<num_blocks, block_size>>>(d_queries, queries.size(), d_boundaries, boundaries.size(), d_results);

    vector<int> results(queries.size());
    cudaMemcpy(results.data(), d_results, queries.size() * sizeof(int), cudaMemcpyDeviceToHost);
